...
inline float FetchRawDepth(float2 uv) {
    ...
    return SampleCameraDepth(uv * _TargetScale.xy);
}
...
inline float3 FetchViewNormals(float2 uv, float2 delta, float3 P) {
    ...
    #else
    uint2 positionSS = uv * _TargetScale.xy * _Input_TexelSize.zw;
    NormalData normalData;
    DecodeFromNormalBuffer(positionSS, normalData);
    float3 N = TransformWorldToViewDir(normalData.normalWS);
    N = float3(N.x, -N.yz);
    #endif
    return N;
}
