...
namespace HorizonBasedAmbientOcclusion.HighDefinition
{
    ...
    public class HBAOEditor : VolumeComponentEditor
    {
        ...
        public override void OnEnable()
        {
            ...
            m_HBAO = (HBAO)target;
            m_HBAOTex = Resources.Load<Texture2D>("hbao_hdrp");
            ...
            EditorGUILayout.BeginVertical();
            {
                ...
                foreach (var group in m_GroupFields)
                {
                    ...
                    foreach (var field in group.Value)
                    {
                        ...
                        /*else if (group.Key.GetType() == typeof(HBAO.GeneralSettings) && field.displayName == "debugMode")
                        {
                        ...
                        if (group.Key.GetType() == typeof(HBAO.AOSettings) && field.Name == "radius")
                        {
                        ...
                        else if (group.Key.GetType() == typeof(HBAO.TemporalFilterSettings) && field.Name == "temporalFilterEnabled")
                        {
                            ...
                            else
                            {
                                ...
                                if (!renderpipelineAsset.currentPlatformRenderPipelineSettings.supportMotionVectors)
                                {
                                    GUILayout.Space(6.0f);
                                    EditorGUILayout.HelpBox("Motion vectors are disabled in pipeline asset...", MessageType.Warning);
                                    m_HBAO.EnableTemporalFilter(false);
                                }
                            }
                        }
